...
npm run lint       # ESLint
npx prisma migrate dev --name <name>   # Create a new migration — CURRENTLY BROKEN, see below
...
- **`prisma migrate dev` cannot create migrations in this repo.** It fails with `P3006`/`P3018 — relation "_prisma_migrations" does not exist` while replaying `20260714000000_enable_rls_remaining` against Prisma's *shadow* database: that migration's last statement is `ALTER TABLE "_prisma_migrations" ENABLE ROW LEVEL SECURITY`, and the shadow DB has no such table. It applies fine to the real database, so only migration *authoring* is affected. Until it's fixed, hand-write the migration and apply it:
  ```bash
  mkdir -p prisma/migrations/$(date +%Y%m%d%H%M%S)_<name> && $EDITOR prisma/migrations/*_<name>/migration.sql
  set -a && . ./.env && set +a && npx prisma migrate deploy   # Prisma CLI needs .env loaded explicitly
  ```
  Do **not** "fix" it by editing `20260714000000_enable_rls_remaining` — it is already applied, and changing the file changes its checksum. A proper fix is a new migration making that statement conditional.
- **Adding a new model:** two manual steps Prisma won't generate for you. (1) Add it to `TENANT_SCOPED_MODELS` in `lib/db.ts` if it's tenant-scoped. (2) **Enable RLS on the new table** in the same migration: `ALTER TABLE "NewTable" ENABLE ROW LEVEL SECURITY;`. The cloud database is Supabase, where every `public` table is reachable via PostgREST/GraphQL with the public anon key. RLS with no policies is deny-all there and a no-op for the app (Prisma connects as the table owner, which bypasses RLS). `anon`/`authenticated` also have all privileges revoked (see `20260714010000_revoke_anon_grants`), but keep RLS as defense-in-depth. Skipping this is what caused the Supabase advisor to flag 12 tables in July 2026.
  - **Do not write RLS policies.** The Supabase advisor reports `rls_enabled_no_policy` (INFO) for every one of these tables. That is expected and correct — policy-free RLS is deny-all, which is exactly what we want, and Prisma is unaffected because it connects as the owner. Adding a policy would only *grant* access to roles that should have none. Leave the INFO lint standing.
...
- **`public/*-widget.js` are generated, not source.** `public/announcement-widget.js`, `public/changelog-widget.js` and `public/glintpost-targeting.js` are built from `widgets/*.js` by `scripts/build-widgets.mjs` (`npm run build:widgets`, the first step of `npm run build`) and carry a "do not edit here" banner. Editing the `public/` copy appears to work until the next build silently overwrites it. Edit `widgets/`, then rebuild. (`public/feedback-widget.js`, `public/roadmap-widget.js` and `public/widget.js` are still hand-written.) ESLint ignores the three generated bundles — the sources are linted instead, since esbuild's `es2017` target downlevels `catch {}` into an unused `catch (e) {}`.
- **Rich-text media is inline-only.** Images upload through the editor into `content`; video is embed-by-link (Quill emits `<iframe class="ql-video">`) — there is no video upload, and `/api/upload` accepts images only. Use `hasRichContent()` from `lib/rich-text.ts` for "is this document empty" checks: stripping tags alone reports an image-only or video-only document as empty and blocks saving. It counts only `img`/`iframe`, matching what survives `sanitizeRichHtml` — counting `<video>` would let a document save and then publish as empty. Public surfaces must size embedded media themselves; Quill's stylesheet is loaded only in the dashboard editor.
- **Adding a new feature/widget:** Register it in `lib/widgets.ts` (both `WIDGETS` and/or `WIDGETS_WITH_FEEDBACK`), create its public page under `app/`, create its widget script in `widgets/` (bundled to `public/`), add dashboard pages under `app/(dashboard)/`, and add API routes under `app/api/`. **Allowlist the new public page + public (API-key) routes in `proxy.ts` `publicPaths`** — otherwise the auth middleware (`proxy.ts`, Next 16's renamed `middleware.ts`) redirects cross-origin widget requests to `/login`. Do NOT allowlist dashboard/session routes there (they stay protected and rely on `requireOrgApi`).
...
- **Styling: reach for the global primitive first.** `app/globals.css` owns the shared vocabulary — `btn-*`, `card`, `list`/`list-row*`, `page*`, `field`/`field-label`/`field-hint`, `input-field`, `form-error`, `badge`, `empty-state`, `dialog*`, `text-muted`/`text-danger`. Use them as plain `className="field-hint"` strings; a CSS Module should only hold layout and genuinely page-specific rules. Do not re-declare a global under a local name (a module `.hint`/`.card`/`.emptyState` that just restates the global is the smell). If a primitive needs a variant, change it in `globals.css` rather than forking it locally. _(Partially migrated — several modules still shadow `.card`, `.emptyState`, `.container` and `.inputField`; prefer the global when touching them.)_ The embeddable widgets in `widgets/` are the exception: they render on the customer's page where `globals.css` does not exist, so they inline their own CSS.
