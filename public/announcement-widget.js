(function () {
  ...
  function matchesTargeting(announcement) {
    if (!announcement.targetingRules) return true;
    if (!visitorDatalayer) return false;
    var rules = announcement.targetingRules.rules;
    var op = announcement.targetingRules.operator;

    function evalRule(rule) {
      var rawValue = visitorDatalayer[rule.param];
      var actual = (rawValue !== undefined && rawValue !== null) ? String(rawValue) : "";
      switch (rule.op) {
